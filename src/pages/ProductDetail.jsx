...
function ProductDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      //   `${import.meta.env.VITE_DEPLOY_PORT}/products/${id}`
      // );
  ...
  const increaseQuantity = () =>
    setQuantity((prev) => Math.min(prev + 1, displayQuantity));
  ...
  const handleSelectColor = (color) => {
    ...
    const variant = product.variants.find(
      (v) => v.image.color === color && v.quantity > 0
    );
  ...
  const buyNow = async () => {
    if (!selectedColor || !selectedSize)
      return toast.error("Thiếu trường", "Vui lòng chọn đủ các trường giá trị");
    const res = await api.post(`/cart/buyNow`, {
      variant_id: selectedVariant._id,
      quantity,
    });
    if (res.status === 200) {
            ...
            <div className="mt-4">
              <p className="font-semibold mb-2">
                ...
                <span className="text-gray-700">
                  {selectedVariant ? selectedVariant.quantity : displayQuantity}
                </span>
              </p>
            </div>
            <div className="mt-4">
              <p className="font-semibold mb-2">
                Giá:{" "}
                <span className="text-gray-700">
                  {display.toLocaleString("vi-VN")} VNĐ
                </span>
              </p>
            </div>
